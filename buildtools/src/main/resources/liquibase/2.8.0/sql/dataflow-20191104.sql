...
FROM (
         (SELECT
            ...
            coalesce(
                (select params.string_val from dataflow.batch_job_instance ins
                  join dataflow.batch_job_execution exe on ins.job_instance_id = exe.job_instance_id
                  join dataflow.batch_job_execution_params params on params.job_execution_id = exe.job_execution_id and params.key_name in ('regionGroup','rg')
                  where ins.job_instance_id = (select long_val from dataflow.batch_job_execution_params jep3
