...
CREATE VIEW dataflow.vw_stl_jobs AS
  ...
  FROM (
           ...
           left join (select billing_period as adj_billing_period, adjustment_no, region_group as rg, job_id from dataflow.map_billing_period_adjustment_no group by billing_period, adjustment_no, region_group, job_id) adj_no
             on old_q.billing_period = adj_no.adj_billing_period
                and old_q.region_group = adj_no.rg
                and coalesce((string_to_array(old_q.job_name, '-'  ))[2]::numeric, 0) = adj_no.job_id
                and old_q.process_type = 'ADJUSTED') inner_q;
